...
\title{A Schedule-Robust $\Omega(d)$ Hitting-Time Separation\\
between Stable Gradient Descent and Spectral-Frank-Wolfe}
\date{}
...
\begin{abstract}
We study a over-capacity Gaussian associative-memory loss with Zipf weights, where
$N=\operatorname{poly}(d)$ and $N\gtrsim d^2$.  On the target accuracy range
$\frac{(\log N)^\alpha}{d^{2(\alpha-1)}}
 \lesssim\eps\lesssim
 \frac{\log N}{d^{\alpha-1}},
$ the standard Frank-Wolfe
smoothness-radius certificate for NGD is a factor $d$ larger than the Muon
 certificate.  We also prove an actual $\Omega(d)$ hitting-time separation
 between any fixed stable GD schedule prescribed before the sample, with
 steps at most $3$, and Muon-Frank-Wolfe.  Finally, we show that the Muon's hitting-time order remains valid after the effective learning rate is capped by an arbitrary fixed positive constant.
\end{abstract}
...
\[
 \Sb_t^X
 \in\mathop{\arg\min}_{\|\Sb\|_X\leq R_X(\eps/2)}
...
\label{thm:fw}
Fix $q>0$.  There exist $C_{\mathrm{low}},c_{\mathrm{up}}>0$ such that,
...
\[
 \frac{\HF R_{\mathrm F}(\eps/2)^2}{\eps}
 \eqsim (\log N)
 \left(\frac{\log N}{\eps}\right)^{\alpha/(\alpha-1)} \eqsim
 d\,\frac{\Hop R_{\mathrm{op}}(\eps/2)^2}{\eps}.
\]
\end{theorem}

\begin{theorem}[GD lower bound and actual $\Omega(d)$ separation]
\label{thm:gd}
Fix $q>0$.  There exist $C_{\mathrm{low}},c_{\mathrm{up}}>0$, which may be
chosen so that \Cref{thm:fw} holds on the same range, such that for every
target satisfying
$
 C_{\mathrm{low}}
 \frac{(\log N)^\alpha}{d^{2(\alpha-1)}}
 ...
 \frac{\log N}{d^{\alpha-1}},
$
and every deterministic schedule prescribed before the sample satisfying
$0\leq\eta_t(\eps)\leq3$, the following holds with probability at least
$1-O(d^{-q})$:
\[
 \tau_{\mathrm{GD}}(\eps)
 \gtrsim
...
\label{thm:capped}
Fix $q>0$ and any constant $\bar\eta>0$.  There exist
$C_{\mathrm{low}},c_{\mathrm{up}}>0$, which may be chosen so that
\Cref{thm:fw} holds on the same range, such that for every target in the range
$
 C_{\mathrm{low}}
 \frac{(\log N)^\alpha}{d^{2(\alpha-1)}}
 \leq\eps\leq
 ...
 \frac{\log N}{d^{\alpha-1}}.
$
In the same Muon-Frank-Wolfe recursion, replace only the mixing schedule
by
\[
 \gamma_t=\frac2{t+2+a},
 \quad
 a=\max\!\left\{
 0,\frac{2R_{\mathrm{op}}(\eps/2)}{\bar\eta}-2
 \right\}.
\]
Write $\tau_{\mathrm{Muon}}(\eps;\gamma)$ for the hitting time of this same
recursion with the displayed schedule.  Then, for every $t\geq0$,
\[
 \gamma_tR_{\mathrm{op}}(\eps/2)\leq\bar\eta,
 \qquad
 \normop{\Wb_{t+1}^{\mathrm{op}}-\Wb_t^{\mathrm{op}}}
 \leq2\bar\eta.
\]
With probability at least $1-O(d^{-q})$,
\[
 \tau_{\mathrm{Muon}}(\eps;\gamma)
 \lesssim_{\bar\eta}
 \frac{\Hop R_{\mathrm{op}}(\eps)^2}{\eps}
 \lesssim_{\bar\eta}
 \frac{\log N}{d}
 \left(\frac{\log N}{\eps}\right)^{\alpha/(\alpha-1)}.
\]
...
\]
with a sufficiently large absolute constant $C$ (e.g.\ $C=3\sqrt{2(q+5)}$)
...
\begin{align*}
 ...
 \left(\ub_i-\frac1N\sum_{j=1}^N\ub_j\right)^\top\Wb\vb_i\\
 &\qquad\geq
 K\log N-\sum_{i=1}^K\ell_i(\Wb)\\
 &\qquad\geq\frac78K\log N.
\end{align*}
...
\begin{align*}
 ...
 &=T_t(\Wb_t^{\mathrm{GD}})-T_t(\widetilde\Wb_t^{(B)})-\eta_t(\eps)\nabla L_B(\widetilde\Wb_t^{(B)}).
\end{align*}
...
\begin{align*}
 ...
 &\leq\normF{\Wb_t^{\mathrm{GD}}-\widetilde\Wb_t^{(B)}}+\eta_t(\eps)
 \sup_{\Zb}\normF{\nabla L_B(\Zb)}\\
 &\leq\normF{\Wb_t^{\mathrm{GD}}-\widetilde\Wb_t^{(B)}}
 +O\!\left(
  \eta_t(\eps)\left(\sum_{i\in B}p_i^2\right)^{1/2}
 \right).
\end{align*}
...
$(\eta_s(\eps))_{s\ge0}$ is prescribed before the sample is drawn, it is
...
%  \right]-
%  O\!\left(
%   \left(\sum_{s<t}\eta_s(\eps)\right)Kp_K^2
%  \right).
% \end{align*}
% If
% \[
...
% Because $\eta_s(\eps)\leq3$,
% \[
%  \sum_{s<t}\eta_s(\eps)\leq3t.
% \]
...
Since $0\leq\eta_s(\eps)\leq3$, whenever $t$ is at most a sufficiently
small fixed multiple of $(\log N)/p_K$, we have
$
 \sum_{s<t}\eta_s(\eps)\leq 3t \lesssim\frac{\log N}{p_K}.
$
